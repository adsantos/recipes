...
window.add_new_step = (target_id, step_id) ->
  ...
  id = $("#{target_id} > p").size()
  # Create a preview on the right hand side of the page
  $(target_id).append("<p id='recipe_steps_#{id}_p' name='recipe[steps][]' class='step'>#{step}  <a href='#' class='recipe_link' onclick=remove_from_preview('#recipe_steps_#{id}')> x</a></p>")
  # Add the value to the hidden field so that the server receives it
